...
float ABossEnemy_Rogue::TakeDamage(float a_damageAmount, FDamageEvent const& a_damageEvent, AController* a_eventInstigator, AActor* a_damageCauser)
{
	if (!m_isInvulnerable)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(m_invulnarabilityTimerHandle, this, &ABossEnemy_Rogue::MakeThisVulnerable, m_invulnerableTime, false);
	}
